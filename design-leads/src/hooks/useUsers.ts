...
/** Just the designers -- for assignment dropdowns and per-designer KPI/attendance views. */
export const useDesignersList = () => {
  const { data, ...rest } = useUsersList();
  return { data: data?.filter((u) => u.role === "designer"), ...rest };
};
