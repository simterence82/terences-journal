  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isFirstRun) {
        ...
        // meta/setup sentinel that marks bootstrap as done. Firestore
        // security rules allow exactly this one self-created admin doc, and
        // only while meta/setup doesn't exist.
        ...
        // Anyone can create a Firebase Auth account -- what actually grants
        // access is an admin approving the request (as a designer or
        // another admin), which creates users/{uid}. Until then this
        // account can sign in but has no role doc, so it stays stuck on the
        // "pending" screen.
        const cred = await createUserWithEmailAndPassword(auth, email, password);
        await updateProfile(cred.user, { displayName });
      ...
      <div className="w-full max-w-md overflow-hidden rounded-lg border border-border bg-card shadow-lg">
        ...
        <div className="p-6 sm:p-8">
          <div className="mb-2 flex items-center gap-3">
            ...
            <h1 className="font-display text-2xl font-semibold text-foreground">Studio Leads</h1>
          </div>
