...
export function gradeForScore(score: number | null): Grade | null {
  ...
  if (score >= 90) return { letter: "A+", label: "Outstanding", tone: "success" };
  if (score >= 80) return { letter: "A", label: "Excellent", tone: "success" };
  if (score >= 65) return { letter: "B", label: "Solid", tone: "primary" };
  if (score >= 50) return { letter: "C", label: "Needs Improvement", tone: "warning" };
  return { letter: "D", label: "At Risk", tone: "destructive" };
}
